...
function addMarker(place)
{
    ...
    // Create marker at location "place"
    ...
    // Get postal code of place
    let parameters = {
        geo: place.postal_code
    };

    // Add info window pop up functionality
    marker.addListener("click", function() {
        ...
        $.getJSON("/articles", parameters, function(data, textStatus, jqXHR) {
            ...
            if (data.length < 1)
            {
                infoWindowHTML = "<h>No news available for this area</h>";
            }

            // Create list of the available links
            else
            {
                infoWindowHtml = "<ul>";
                ...
                for (let i = 0; i < 5; i++)
                {
                    infoWindowHtml += "<li><a href=" + data[i].link + ">" + data[i].title + "</a></li>";
                }
            ...
            // Build info window
            showInfo(marker, infoWindowHtml);
        });
    });
...
function removeMarkers()
{
    ...
    {
        // Marker is in map
        if (isInbounds(markers[i]))
        {
            inMarkers.push(markers[i]);
        }
        else
        {
            // set markers to null
